...
unsigned long measureFunction(float compare_to, double (*test_func)(float)) {
  float_x xbase;
  unsigned int dummy;
  ...
  do{
    ...
    for (count = 0x0; count < 0x10000000; count++) {
      xbase.x = count;
      rlibm_rno_res = test_func(xbase.f);
  ...
  printf("someAccumulator = %lu\n", someAccumulator);
  rlibm_rno_total = (rlibm_rno_t2 - rlibm_rno_t1);
  return rlibm_rno_total;
}  
...
int main(int argc, char** argv){
  ...
  for(rlibm_func rf  = LOG; rf <= SINH; rf++){
    set_function_ptr(rf);
    unsigned long rlibm_rno_total = measureFunction(compare_to, f1);
    printf("rno cycle = %lu\n", rlibm_rno_total);
    unsigned long rlibm_prog_total = measureFunction(compare_to, f1);
