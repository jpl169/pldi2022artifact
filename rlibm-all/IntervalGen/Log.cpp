...
/* return value is a true (special case) and false (otherwise) */
...
double IntervalGenerator::OutputCompensation(float x, double yp) {
    ...
    /* make all bits other than the first 7 bits of mantissa zero */
    fit.x = fix.x & 0x007F0000;

    /* FIndex should be between 0 to 127 */
    int FIndex = fit.x >> 16;
    ...
    double extra_factor = fma(m, LN2HIGH, lnLutHIGH[FIndex]);
