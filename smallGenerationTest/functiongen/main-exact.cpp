...
polynomial* rlibm_solve_with_soplex(DVectorRational& coeffs, sample_data* sintervals,
...
Rational rlibm_poly_evaluation(DVectorRational& coeffs, double x_input, polynomial* poly){

  Rational ret_val = 0.0, x(x_input);
...
bool rlibm_validate_intervals(DVectorRational& coeffs, sample_data* sintervals,
			      size_t ssize, polynomial* poly){

  bool return_val = true;
  for(size_t i = 0; i < ssize; i++){
    Rational y = rlibm_poly_evaluation(coeffs, sintervals[i].x, poly);

    if((y < Rational(sintervals[i].orig_lb)) || (y > Rational(sintervals[i].orig_ub)))
        return_val = false;
  }
  return return_val;
}
...
polynomial*
rlibm_generate_polynomial(DVectorRational& coeffs, sample_data* sintervals, size_t ssize,
  ...
  for(int i = prev_successful_degree; i < power_size; i++){
    ...
    while(count < max_tries){
      polynomial* p = rlibm_solve_with_soplex(coeffs, sintervals, ssize, power, i+1);
      if(p && rlibm_validate_intervals(coeffs, sintervals, ssize, p)){
...
size_t rlibm_compute_violated_indices(DVectorRational& coeffs, size_t* violated_indices, interval_data* intervals, size_t nentries, polynomial* poly){

  size_t num_violated_indices = 0;
  for(size_t i = 0; i < nentries; i++){
    Rational y = rlibm_poly_evaluation(coeffs, intervals[i].x, poly);
    if( y < Rational(intervals[i].lb) || y > Rational(intervals[i].ub)){
...
void rlibm_print_polyinfo(polynomial* p, DVectorRational& coeffs){
  ...
  gmp_printf("Polynomial: y=%Qd x^(%d)",coeffs[0],p->power[0]);
  for(int j=1;j<p->termsize;j++){
    gmp_printf(" + %Qd x^(%d)",coeffs[j],p->power[j]);
  }
  printf("\n");
}
...
int main(int argc, char** argv){
  ...
  DVectorRational coeffs(powers_size);
  ...
  do{
    ...
    p = rlibm_generate_polynomial(coeffs, sampled_intervals, samplesize, powers, powers_size, MAX_TRIES);

    if(p){
      n_violated_indices = rlibm_compute_violated_indices(coeffs, violated_indices, intervals, nentries, p);
